...
import './stories/Wordle.css';
import './main.scss';
import './styles.scss';
import Confetti from "react-confetti";
import { useDispatch } from "react-redux";
import {endTime} from "../../state/index";
...
const Stage1 = ({rounds}) => {
  ...
  const handelbutton = async (rounding)=>{
    ...
    const savedUserResponse =  fetch(
      `http://localhost:3001/users/${userId}/rounds`,
      {
        ...
        body: JSON.stringify({id:userId,round: rounding,timesp:time}),
      }
    );
  ...
  useEffect(() => {
    const calcpoints = async () => {
      ...
      const {listRound,timesu} = await response.json();
  
      let unique = listRound.filter((item, i, ar) => ar.indexOf(item) === i);
      console.log("unique ",unique)
      let pos = 0;
      unique.includes(1)?pos = pos + 150:pos = pos;
      unique.includes(2)?pos = pos + 100:pos = pos;
      unique.includes(3)?pos = pos + 150:pos = pos;
      unique.includes(4)?pos = pos + 150:pos = pos;
    ...
    <div>
      {finish && <Confetti />}
     {
      ...
      <div onClick={()=>{
        handelbutton(1)
        setStart(true)
      }} className={!start?"button ssl button--flex":"button sslp button--flex"}>
                Start 
                <svg
                  class="button__icon"
     ...
     <div onClick={()=>{
        ...
        <Hanoi userId={userId} token={token} />
        <div onClick={()=>{
        handelbutton(4)
      }} className="button ssl button--flex">
                Next 
     ...
     <Tenzies userId={userId} token={token}/>
     <div onClick={()=>{
        handelbutton(5)
      }} className="button ssl button--flex">
                Next 
     ...
     </>:roun == 5?<><Diffrence  userId={userId} token={token} />
     ...
     <Wordle
        ...
        handelbutton(8),
        handelfinish()
      }} className="button ssl button--flex" style={{backgroundColor:"#BF1717"}}>
                Finish 
                <svg
                  class="button__icon"
                  xmlns="http://www.w3.org/2000/svg"
        ...
        <br/>
        <h2>
          Time Spent ⌚: {intime} minutes
        </h2>
        <br/>
        <div onClick={moveleader} className="button ssl button--flex lead">
                LeaderBoard 
                <svg
                  class="button__icon"
                  xmlns="http://www.w3.org/2000/svg"
